...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    softmax_result = softmax(np.dot(center_word_vec,outside_vectors.T))
    loss = -np.log(softmax_result[outside_word_idx])
    softmax_result[outside_word_idx] -= 1
    grad_center_vec = np.dot(outside_vectors.T,softmax_result.reshape((-1, 1)))
    grad_outside_vecs = np.outer(softmax_result, center_word_vec.reshape((-1, 1)))
    ### END YOUR CODE
...
def neg_sampling_loss_and_gradient(
    ...
    inside_sigmoid = np.dot(center_word_vec, outside_vectors[outside_word_idx])
    sig = sigmoid(inside_sigmoid)
    loss = -np.log(sig)
    # -(1 - sigmoid(center_word_vec^T*outside_vectors[outside_word_idx])) * outside_vectors[outside_word_idx]
    grad_center_vec = (sig - 1) * outside_vectors[outside_word_idx]
    # -(1 - sigmoid(center_word_vec^T*outside_vectors[outside_word_idx])) * center_word_vec
    grad_outside_vecs[outside_word_idx] = (sig - 1) * center_word_vec

    # Calculate for k negative samples
    for k in neg_sample_word_indices:
        inside_sigmoid = np.dot(center_word_vec, outside_vectors[k])
        sig = sigmoid(-inside_sigmoid)
        loss -= np.log(sig)
        grad_center_vec += (1 - sig) * outside_vectors[k]
        grad_outside_vecs[k] += (1 - sig) * center_word_vec
    ### END YOUR CODE
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        ...
        sum_loss, cur_grad_center_vec, cur_grad_outside_vecs = word2vec_loss_and_gradient(center_word_vec, outside_word_idx, outside_vectors, dataset) 
        loss += sum_loss
        # updating gradient for current center word
        grad_center_vecs[center_word_idx] += cur_grad_center_vec.reshape((1,-1))[0]
        # updating gradient for outside words
