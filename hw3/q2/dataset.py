...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        length_for_truncate = random.randint(4, int(self.block_size * 7 / 8))
        truncated_document = document[:length_for_truncate]
        ...
        masked_content_length = len(truncated_document) // 4
        range_masked_content_length = masked_content_length + random.randint(0 - masked_content_length, 0 + masked_content_length)
        cut_index = random.randint(0,range_masked_content_length)


        masked_content = truncated_document[cut_index:cut_index + range_masked_content_length]
        prefix = truncated_document[:cut_index]
        suffix = truncated_document[range_masked_content_length+cut_index:]

        # 3. Rearrange these substrings into the following form:
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]

        masked_prefix = prefix + self.MASK_CHAR
        masked_suffix = suffix + self.MASK_CHAR
        masked_string = masked_prefix + masked_suffix + masked_content
        ...
        masked_string_input = masked_string[:-1]
        masked_string_output = masked_string[1:]

        # 5. Making use of the vocabulary that you defined, encode the resulting input
        # and output strings as Long tensors and return the resulting data point.

        x = torch.tensor([self.stoi[c] for c in masked_string_input], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in masked_string_output], dtype=torch.long)
