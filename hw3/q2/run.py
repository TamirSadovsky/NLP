...
if __name__ == "__main__":
    ...
    if args.function == 'pretrain':
        ...
        finetune_corpus_TSV_file  = open(args.finetune_corpus_path).read()
        finetune_corpus_for_model  = dataset.NameDataset(pretrain_dataset, finetune_corpus_TSV_file)

        if args.eval_corpus_path is None:
            tconf = trainer.TrainerConfig(**parameters)
            trainer.Trainer(model, finetune_corpus_for_model, None, tconf).train()
            torch.save(model.state_dict(), args.writing_params_path)

        else:
            eval_corpus = open(args.eval_corpus_path).read()
            tconf = trainer.TrainerConfig(**parameters)
            trainer.Trainer(model, finetune_corpus_for_model, dataset.NameDataset(pretrain_dataset, eval_corpus), tconf).train()
            torch.save(model.state_dict(), args.writing_params_path)
