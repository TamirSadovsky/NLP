...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    # Calculate the start point for the current batch
    batch_start = np.random.choice(np.arange(len(in_word_index)))

    data_samples = np.asarray([num_to_word_embedding[in_word_index[idx % len(in_word_index)]]
                               for idx in range(batch_start, batch_start + BATCH_SIZE)])
    label_samples = np.asarray([int_to_one_hot(out_word_index[idx % len(in_word_index)], output_dim)
                                for idx in range(batch_start, batch_start + BATCH_SIZE)])

    # Compute cost and gradient via forward and backward propagation
    cost, grad = forward_backward_prop(data_samples, label_samples, params, dimensions)
...
def eval_neural_lm(eval_data_path):
    ...
    probs = np.zeros(num_of_examples)

    for i in range(num_of_examples):
        in_word = num_to_word_embedding[in_word_index[i]]
        out_word = out_word_index[i]
        prob = forward(in_word, out_word, params, dimensions)
        probs[i] = prob
